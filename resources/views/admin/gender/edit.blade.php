...
<div class="main-container-middle">
    <div class="container-header container-customer" style="overflow:visible;">
        <div class="d-flex align-items-center justify-content-between p-3">
            <div class="h5 m-0">
                <a href="{{ url('admin/gender/list') }}" class="btn btn-outline-primary mr-3" title="{{ Helper::trans('gender.back_to_[object Object]', 'Back to gender list') }}"><i class="fas fa-arrow-left"></i></a>
                <span>{{ Helper::trans('gender.edit_title', 'Edit gender') }} ( {{ $model_info->gender_name }} )</span>
            </div>
            <div>
                <a href="{{ url('admin/gender/create') }}" class="btn btn-primary"><i class="fas fa-plus-circle mr-1"></i> {{ Helper::trans('general.add_new', 'Add new') }}</a>
                <!-- <a href="{{ url('admin/gender/list') }}" class="btn btn-primary"><i class="fas fa-list mr-1"></i> {{ Helper::trans('general.list', 'List') }}</a> -->
                <a href="{{ url('admin/gender/delete/'. $model_info->id) }}?redirect={{ url('admin/gender/list') }}" class="btn btn-danger" data-confirmation='{{ Helper::trans('general.delete_alert', 'I you sure, you want to delete') }} "{{ $model_info->gender_name }}"?'><i class="fas fa-trash mr-1"></i> {{ Helper::trans('general.delete', 'Delete') }}</a>
            </div>
        </div>
    </div>
    <div class="container-detail">
        <div class="container-customer">
            <div class="px-3 mb-3">
                <div class="card card-body bg-white p-5">
{!! \App\Handlers\Admin\Gender\EditFormHandler::handler(request(), $model_info->id, false) !!}
                </div>
            </div>
        </div>
    </div>
